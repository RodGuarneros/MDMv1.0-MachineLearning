...
with tab6:
    ...
    with st.expander('Análisis', expanded=False):
        st.markdown(f'La clasificación proporcionada por el aprendizaje automático no supervizado sugiere que <span style="color:#51C622"> la madurez digital no es aleatoria, sino que sigue patrones relacionados con factores financieros, socio-económicos y geográficos</span>. De hecho, cuando se realizaba el entrenamiento de los modelos y se evaluaban se revisó la matriz de pesos de cada variable y la latitud y longitud son variables relevantes dentro del modelo, seguidos de el número de vecionos cercanos en un radio de 5km y el número de vecinos. Sugiriendo que la proximidad geográfica entre los municipios influye en su madurez digital debido a la infraestructura compartida y la mobilidad de sus factores productivos.</span', unsafe_allow_html=True)
        st.markdown(f'Es evidente en el mapa una <span style="color:#51C622">concentración de municipios con nivel de madurez óptima (color verde) al rededor de zonas metropolitanas y norte del país.</span>', unsafe_allow_html=True)
